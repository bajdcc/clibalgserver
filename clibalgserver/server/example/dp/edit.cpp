...
void idle3(char* a, char* b, char* c) {
    trace_rapid(1);
    *a = *a;
    *b = *b;
    *c = *c;
...
int main(int argc, char** argv) {
    int i, j;
    char* a = "racket"; int na = strlen(a) + 1;
    char* b = "stack";int nb = strlen(b) + 1;
    ...
    for (i = 1; i < na; i++) {
        for (j = 1; j < nb; j++)  {
            ...
            if (a[i - 1] == b[j - 1]) {
                idle_(&(dp[(i-1) * nb + j-1]));
                dp[i * nb + j] = dp[(i-1) * nb + j-1];
            }
            else {
                idle3(&(dp[(i-1) * nb + j]), &(dp[(i) * nb + j-1]), &(dp[(i-1) * nb + j-1]));
                dp[i * nb + j] = min(min(dp[(i-1) * nb + j], dp[(i) * nb + j-1]), dp[(i-1) * nb + j-1]) + 1;
            }
        }
    }
